...
class Bm1000cGattCallback(listener: GattCallbackListener): DeviceGattCallback(listener) {
    private var lastSend = Date().time / 1000
    fun setMac(mac: String): Bm1000cGattCallback {
        this.mac = mac
        return this
    }
    ...
    override fun initCharacteristic(gatt: BluetoothGatt) {
        ...
        val characteristic = services.firstOrNull { service ->
            return@firstOrNull if (service?.uuid == Const.UUID_SERVICE_PO) {
                val characteristic = service?.characteristics?.firstOrNull { characteristic ->
                    characteristic.uuid == Const.UUID_PO_CHARACTER_RECEIVE
                }
                characteristic != null
            } else {
                false
            ...
            char.uuid == Const.UUID_PO_CHARACTER_RECEIVE
        }
    ...
    override fun sendData(array: ByteArray, gatt: BluetoothGatt) {
        ...
        listener.onData(DeviceType.BM1000C, array, gatt, mac!!)
    }
}